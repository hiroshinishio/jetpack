 ...
 */
export default function useIgnoreThreatMutation(): object {
	const queryClient = useQueryClient();
	const { setNotice } = useDispatch( STORE_ID );

	return useMutation( {
		mutationFn: threatId => API.ignoreThreat( threatId ),
		onSuccess: () => {
			...
			setNotice( {
				type: 'success',
				message: __( 'Threat ignored.', 'jetpack-protect' ),
			} );
		},
		onError: () => {
			...
			setNotice( {
				...
				message: createInterpolateElement(
					__(
						'An error occurred ignoring the threat. Please try again or <supportLink>contact support</supportLink>.',
						'jetpack-protect'
					),
					{
